...
const CompileButton: React.FC<Props> = ({ room, token, onPdf, onToast, onLog, onStatus }) => {
  const { ytext } = useCollabDoc(room, token);
  const [status, setStatus] = useState<CompileStatus | 'idle'>('idle');

  const handleClick = async () => {
    try {
      const jobId = await startCompile(token, ytext.toString());
      setStatus('PENDING');
      onStatus('PENDING');
      const es = new EventSource(`${API_URL}/stream/jobs/${jobId}?project=${token}`);
      es.onmessage = async (ev) => {
        ...
        if (msg.status === 'SUCCEEDED') {
          es.close();
          const blob = await fetchPdf(jobId, token);
